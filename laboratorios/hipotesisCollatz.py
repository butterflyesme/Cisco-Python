...
número natural (incluso puede requerir inteligencia artificial), pero puede usar Python para verificar algunos 
...
También queremos que cuente los pasos necesarios para lograr el objetivo. Tu código también debe mostrar todos 
los valores intermedios de c0.
Sugerencia: la parte más importante del problema es como transformar la idea de Collatz en un ciclo 
...
if c0 > 1:
    pasos = 0
    while c0 != 1:
        if c0 % 2 != 0: #Evalua si es impar
            cnew = 3 * c0 + 1
        else:
            cnew = c0 // 2 #Elalua si es par 
        print(cnew)
        c0 = cnew
        pasos += 1 #Evalua cada paso 
    print("pasos =", pasos)
else:
